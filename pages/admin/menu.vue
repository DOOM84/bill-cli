      ...
      <div class="table-responsive mb-5">
        <table class="table table-striped ">
          ...
          <tr v-for="(pic, i) in pictures" :key="pic.id">
            <th scope="row">{{i+1}}</th>
            <td>
              <img @click="showImage(pic.images.original)" style="cursor: pointer" height="200px" :src="pic.images.thumbnail" alt="">
            </td>
...
<script>
  export default {
    ...
    async asyncData({store}) {
      try {
        const {pictures} = await store.dispatch('adminDish/index');
        return {
          pictures
        }
      } catch (error) {
    ...
    methods: {
      ...
      async uploadImgs() {
        ...
        try {
          const {success} = await this.$store.dispatch('adminDish/create', formData);
          this.$notify({
            type: 'success',
            group: 'foo',
            text: success
      ...
      async updateItem(){
        try {
          ...
          const {success, picture} = await this.$store.dispatch('adminDish/update', {id: this.id, status: this.status});
      ...
      async loadGal() {
        const {pictures} = await this.$store.dispatch('adminDish/index');
      ...
      async deleteItem(id) {
        if (confirm('Are You Sure?')) {
          const {success} = await this.$store.dispatch('adminDish/delete', id);
